...
export async function onRequest(context) {
  ...
  try {
    ...
    const { data: devisEnAttente, error: errDevis } = await sb
      .from('devis').select('id, reference, client_email, client_prenom, depart, arrivee, vehicule, total_ht, created_at')
      .eq('status', 'pending').gte('created_at', twoDaysAgo).lte('created_at', yesterday)
...
async function sendRelanceEmail(devis, apiKey) {
  const html = `<div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;background:#FDFBF7;padding:32px;">
    ...
    <h2 style="color:#0A4D68">Votre devis vous attend, ${devis.client_prenom || ''} !</h2>
    <p style="color:#6B625A;font-size:.95rem">Convoyage de ${devis.vehicule || 'véhicule'} de <strong>${devis.depart || ''}</strong> à <strong>${devis.arrivee || ''}</strong>. Prix estimé : <strong>${devis.total_ht || ''}€</strong>.</p>
    <p style="color:#6B625A;font-size:.85rem">Référence : ${devis.reference || ''}</p>
    <div style="text-align:center;margin:32px 0"><a href="https://www.bathily-convoyage.fr/dashboard-client.html" style="background:#0A4D68;color:white;padding:14px 32px;border-radius:40px;text-decoration:none;font-weight:600;display:inline-block">Finaliser ma demande</a></div>
  </div>`;
  await fetch('https://api.resend.com/emails', {
    method: 'POST', headers: { 'Authorization': 'Bearer ' + apiKey, 'Content-Type': 'application/json' },
    body: JSON.stringify({ from: 'Bathily-Convoyage <noreply@bathily-convoyage.fr>', to: devis.client_email, subject: '⏰ Votre devis de convoyage vous attend', html })
  });
}
...
async function sendRappelClientEmail(mission, apiKey) {
  ...
  const html = `<div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;background:#FDFBF7;padding:32px;">
    ...
    <p style="color:#6B625A;font-size:.95rem">Bonjour ${mission.client_nom || ''}, c'est demain, <strong>${dateStr}</strong>. Trajet de <strong>${mission.depart || ''}</strong> à <strong>${mission.arrivee || ''}</strong>.</p>
...
async function sendRappelConvoyeurEmail(mission, apiKey) {
  ...
  const html = `<div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;background:#FDFBF7;padding:32px;">
    ...
    <p style="color:#6B625A;font-size:.95rem">Bonjour ${mission.convoyeur_nom || ''}, mission demain <strong>${dateStr}</strong> : <strong>${mission.depart || ''}</strong> → <strong>${mission.arrivee || ''}</strong>.</p>
