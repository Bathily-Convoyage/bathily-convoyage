...
export async function onRequest(context) {
  ...
  const rl = checkRateLimit(request, 'client-reset-password', 5, 3600000);
  if (rl) return rl;
  ...
  try {
    ...
    if (!clientRow) {
      return jsonResponse({ error: 'Aucun compte client trouvé avec cet email.' }, 404, getCorsHeaders(request));
    }
    ...
    if (!resetUrl) {
      try {
        const { data: inviteData, error: inviteError } = await supabase.auth.admin.generateLink('invite', cleanEmail, { redirectTo });
