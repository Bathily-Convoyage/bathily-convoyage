  ...
  function renderAvis(container, data, limit) {
    ...
    if (!data || data.length === 0) {
      ...
      if (data.length > VISIBLE_COUNT) {
        ...
        html += '<button id="btnToggleAvis" style="background:none;border:2px solid var(--bordeaux);color:var(--bordeaux);padding:10px 28px;border-radius:30px;font-weight:600;font-size:0.85rem;cursor:pointer;transition:all 0.2s;display:inline-flex;align-items:center;gap:8px;">';
      ...
      if (btnToggle) {
        btnToggle.addEventListener('click', function () {
          ...
          var icon = document.getElementById('toggleAvisIcon');
          ...
          if (expanded) {
            ...
            btnToggle.firstChild.textContent = 'Voir les ' + hiddenCards.length + ' autres avis ';
          } else {
            hiddenCards.forEach(function (c) { c.style.display = 'block'; });
            icon.style.transform = 'rotate(180deg)';
            btnToggle.firstChild.textContent = 'Réduire les avis ';
          }
        });
      }
  }
