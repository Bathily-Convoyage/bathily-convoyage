  ...
  try {
    ...
    if (!authUserId) {
      ...
      const { data: inviteData, error: inviteError } = await supabase.auth.admin.inviteUserByEmail(cleanEmail, {
        redirectTo: corsOrigin + '/reset-password.html'
      });

      if (inviteError) {
        // Si l'utilisateur existe deja dans Auth mais pas lie
        const { data: userList } = await supabase.auth.admin.listUsers();
        const existingUser = userList?.users?.find(u => u.email === cleanEmail);
        if (existingUser) {
          authUserId = existingUser.id;
        } else {
          return { statusCode: 400, headers, body: JSON.stringify({ error: 'Impossible de creer le compte: ' + inviteError.message }) };
        }
      } else {
